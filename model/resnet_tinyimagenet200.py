...
class ResnetTinyImageNet200(nn.Module):
    """
    ResNet-like architecture with Class-Wise Convolutions and hierarchical classification support.
    """
    ...
    def __init__(
        ...
        in_channels: int = 3,
        num_class: int = 200,
        planes: Tuple[int, int, int, int] = (200, 400, 800, 1600),
        dropout: float = 0.0,
        bias: bool = False,
        ...
        self.input_shortcut_flag[-1] = False    # last layer doesn't capture a new shortcut
        self.shortcut_flag[-1] = ADD_SHORTCUT   # last layer has an add shortcut
        self.downsample_flag[-1] = False        # last layer doesn't downsample
        ...
        self.layers = nn.ModuleList(
            [
                ...
                *[
                ...
                # Block 4 (16x16 -> 8x8)
                CWConv(in_channels=planes[3], out_channels=planes[3], kernel_size=3, stride=2, padding=1, bias=bias, num_class=num_class, dropout=dropout),
                *[
